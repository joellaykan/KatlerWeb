//Primeira versão do KatlerWeb lançado no dia 25/10/2020
//<input type="text" placeholder="Procurar" class="form-control form-rounded" />
...
function LoadChatList() {
    ...
    db.on('value', function (lists) {
        document.getElementById('lstChat').innerHTML = `<li class="list-group-item">Conversas</li>`;
                    ...
                    if (noti.numChildren() > 0 && Object.values(noti.val())[0].sendFrom === currentUserKey) {
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10" style="cursor:pointer;">
                                    <div class="name">${user.name}
                                        <button class="btn btn-sm btn-defualt" style="float:right;"><i class="fas fa-user-plus"></i> Enviado</button>
                                    </div>
                                </div>
                            </div>
                        </li>`;
                        ...
                        dbNoti.orderByChild('sendFrom').equalTo(data.key).on('value', function (noti) {
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-10" style="cursor:pointer;">
                                    <div class="name">${user.name}
                                        <button onclick="SendRequest('${data.key}')" class="btn btn-sm btn-primary" style="float:right;"><i class="fas fa-user-plus"></i> Enviar solicitação</button>
                                    </div>
                                </div>
                            </div>
                        </li>`;
...
function NotificationCount() {
    ...
    db.orderByChild('sendTo').equalTo(currentUserKey).on('value', function (noti) {
        let notiArray = Object.values(noti.val()).filter(n => n.status === 'Pendente');
...
function PopulateNotifications() {
    ...
    db.orderByChild('sendTo').equalTo(currentUserKey).on('value', function (notis) {
        if (notis.hasChildren()) {
            lst = `<li class="list-group-item">
                            <input type="text" placeholder="Procurar ou iniciar nova conversa" class="form-control form-rounded" />
                        </li>`;
        }
        notis.forEach(function (data) {
            var noti = data.val();
            if (noti.status === 'Pendente') {
                lst += `<li class="list-group-item list-group-item-action">
                            <div class="row">
                                <div class="col-md-2">
                                ...
                                <div class="col-md-10" style="cursor:pointer;">
                                    <div class="name">${noti.name}
                                        <button onclick="Reject('${data.key}')" class="btn btn-sm btn-danger" style="float:right;margin-left:1%;"><i class="fas fa-user-times"></i> Recusar</button>
                                        <button onclick="Accept('${data.key}')" class="btn btn-sm btn-success" style="float:right;"><i class="fas fa-user-check"></i> Aceitar</button>
                                    </div>
                                </div>
                            </div>
                        </li>`;
